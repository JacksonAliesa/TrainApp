...
database.ref().on('child_added', function(snapshot) {
	...
	//set the freqTD variable to 5mins made new variable for the first train time of 10:00
	// use svtrainfreq to use in calculations to get the train freq input
	var firstTime = sv.trainTime;
	var freq = sv.trainFreq;
	//next arrival conversion
	...
	//variable for the next arrival to append to row
	arrivalTD.text(moment(nextTrain).format('hh:mm'));
	//append next arrival TD to new row
  newRow.append(arrivalTD);
  
 
  //variable for mins away TD
